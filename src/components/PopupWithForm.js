...
function PopupWithForm({isOpen, onClose, name, title, buttonText, children, onSubmit}) {
  ...
  <section className={`popup ${isOpen && "popup_opened"}`} id={name}>
    <div className="popup__container">
      <button onClick={handleCloseWithRest} className="popup__close-button zero-button" id={`${name}-close-button`} type="button"></button>
        <h2 className="popup__title">{title}</h2>
        <form action="#" ref={formRef} onSubmit={onSubmit} method="post" name={name} className="popup__form" id={`${name}-form`}>
          {children}
